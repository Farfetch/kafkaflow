namespace KafkaFlow.Client.Producers
{
    ...
    internal class Producer
    {
        ...
        private readonly ConcurrentDictionary<int, ProducerSender> senders = new ConcurrentDictionary<int, ProducerSender>();
        ...
        private ProducerSender GetHostSender(int partitionLeaderId)
        {
            return this.senders.GetOrAdd(
                partitionLeaderId,
                _ => new ProducerSender(
                    this.cluster.GetHost(partitionLeaderId),
                    this.configuration));
        }
