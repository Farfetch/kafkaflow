namespace KafkaFlow.BatchConsume
{
            ...
            IDisposable
    {
        ...
        async Task ISubjectObserver<WorkerStoppedSubject>.OnNotification() => await this.TriggerDispatchAndWaitAsync();
        ...
        private async Task TriggerDispatchAndWaitAsync()
        {
            ...
            if (!this.dispatchTokenSource.IsCancellationRequested)
            {
        ...
        {
            var localDispatchTokenSource = CancellationTokenSource.CreateLinkedTokenSource(context.ConsumerContext.WorkerStopped);

            this.dispatchTokenSource = localDispatchTokenSource;

            this.dispatchTask = Task
                .Delay(this.batchTimeout, localDispatchTokenSource.Token)
                .ContinueWith(
                    _ => this.DispatchAsync(context, next, localDispatchTokenSource),
        ...
        private async Task DispatchAsync(
            ...
            IDisposable localDispatchTokenSource)
        {
            await this.dispatchSemaphore.WaitAsync();

            localDispatchTokenSource.Dispose();
